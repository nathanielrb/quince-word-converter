...
<ul>
  <li>2009–2010 [FG] <a onmouseover="this.href='mailto'+'\x3A'+'frederic.glorieux'+'\x40'+'diple.net'">Frédéric Glorieux</a></li>
</li>
</ul>
...
>
  ...
  <xsl:template match="office:document-content">
    ...
    <!-- Call to remote XSL is understood as a security problem by browser  -->
  <xsl:comment>
    <xsl:text disable-output-escaping="yes"><![CDATA[ <?xml-stylesheet type="text/xsl" href="../../teipub/xsl/tei2html.xsl"?> ]]></xsl:text>
  </xsl:comment>
  ...
  <xsl:template match="text:p">
    ...
    <xsl:variable name="indent">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$n &lt; 0">hanging</xsl:when>
        <!--
      ...
      <xsl:when test="$mapping">
        ...
        <xsl:variable name="col">
          <xsl:choose>
            ...
            <xsl:when test="$style//@fo:background-color != '#ffffff' and $style//@fo:background-color != 'transparent'">
...
<text:p text:style-name="P33">
  ...
  <xsl:template match="draw:image">
    <xsl:variable name="url">
      <xsl:choose>
        <xsl:when test="$filename != '' and contains(@xlink:href, 'media/')">
          <xsl:value-of select="$filename"/>
          <xsl:text>-img/</xsl:text>
          <xsl:value-of select="substring-after(@xlink:href, 'media/')"/>
        </xsl:when>
        <xsl:when test="$filename != '' and contains(@xlink:href, 'Pictures/')">
          <xsl:value-of select="$filename"/>
          <xsl:text>-img/</xsl:text>
          <xsl:value-of select="substring-after(@xlink:href, 'Pictures/')"/>
        </xsl:when>
        <xsl:when test="$filename != '' and contains(@xlink:href, $filename)">
          <xsl:value-of select="$filename"/>
